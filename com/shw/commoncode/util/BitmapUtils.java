package com.shw.commoncode.util;
...
public class BitmapUtils {
    ...
    private static Bitmap decodeBitmapFromFile(int width, int height, Uri uri, int rotate) {
        ...
        if (uri != null && "file".equals(uri.getScheme())) {
            ...
            if (options.outWidth == -1 || options.outHeight == -1) {
                Log.e(TAG, "decodeBitmapFromFile options.outWidth == -1 || options.outHeight == -1 ");
                return null;
            }
    ...
    private static Bitmap decodeBitmapFromContent(Context context, int width, int height, Uri uri, int rotate) {
        ...
        try {
            ...
            if (options.outWidth == -1 || options.outHeight == -1) {
                Log.e(TAG, "decodeBitmapFromContent options.outWidth == -1 || options.outHeight == -1 ");
                return null;
            }
            options.inSampleSize = Math.min(options.outWidth / width, options.outHeight / height);
            options.inJustDecodeBounds = false;
            ...
            e.printStackTrace();
            Log.e(TAG, "decodeBitmapFromFile Exception");
        } finally {
